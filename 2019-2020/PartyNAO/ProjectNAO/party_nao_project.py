...
#Import moves 
import crouch 
import diagonal_left
import diagonalright
import doublemovement
import move_forward
import movebackward
import right_arm 
import rotation_foot_left_leg
import rotation_foot_right_leg
...
#--------------------------------------------------------------#
#Select a random song and analize the amplitude
song = md.random_song() 
analized_song=md.analyze_music(song) 
#--------------------------------------------------------------#
print("Using simulated annealing algorithm to find the best sequence for the selected song...")
#--------------------------------------------------------------#
#For each move we extract the minimum and maximum time bewteen the move and every possible goal
#Then we construct the dictionary {key=goal_move, value=(min_time, max_time)}
list_goals_with_crouch=["OB_sit relax", "OB_sit", "OB_stand zero", "OB_stand", "OB_hello", "OB_wipe forehead", "OB_crouch"]
d_min_max={}
...
while times_indexes == []: #This while terminates when a sequence which respects all the contraints of the problem is found. 
    ...
    for i in range(20): #This for allow us to generate 100 possible solutions in order to choose the best one 
        ...
        while len(used_intermediate_moves) <=4: #This while condition allows us to respect the constraint of the problem(" The algorithm A 
            ...
            while list_goals != []: #This while allow us to find a solution for each sub-problem
                ...
                for el in list_goals: #For each goal we find a solution 
                    problem=pnp.partyNAO(next_initial_state, el, dict_moves_time, d_min_max[el], analized_song, pnp.list_fast_moves, pnp.list_normal_moves, pnp.list_slow_moves, total_time)
